...
async function getFleachenFromUserBestellt(userID) {
  ...
  try {
    ...
    var fleachenAlle = await conn.execute(`
    SELECT DISTINCT fc.ARTIKELNR, fc.FKOORDINATENIDLAT, fc.FKOORDINATENIDLNG, fc.POSITIONSPUNKT, PEP.PROBENSTATUS, k.FLAECHENFARBE
    ...
    JOIN BESTELLUNG_ENTHAELT_PRODUKT wp ON wp.ARTIKELNR = p.ARTIKELNR AND wp.KUNDENNUMMER = p.KUNDENNUMMER
    ...
    resultsFleachenDaten.fleachenAlle = fleachenAlle[0]

    var fleachenZuZiehen = await conn.execute(`
    SELECT DISTINCT fc.ARTIKELNR, fc.FKOORDINATENIDLAT, fc.FKOORDINATENIDLNG, fc.POSITIONSPUNKT, PEP.PROBENSTATUS, k.FLAECHENFARBE
    FROM FLEACHENKOORDINATE fc
    JOIN PRODUKT p ON p.ARTIKELNR = fc.ARTIKELNR AND p.KUNDENNUMMER = fc.KUNDENNUMMER
    JOIN PRODUKT_ENTHAELT_PROBE PEP ON p.ARTIKELNR = PEP.ARTIKELNR AND p.KUNDENNUMMER = PEP.KUNDENNUMMER
    ...
    JOIN BESTELLUNG w ON w.KUNDENNUMMER = wp.KUNDENNUMMER
    JOIN KUNDE k ON k.KUNDENNUMMER = w.KUNDENNUMMER
    WHERE fc.KUNDENNUMMER = ? AND PROBENSTATUS = 1
    ORDER BY fc.ARTIKELNR, fc.POSITIONSPUNKT  
    `, [userID]);

    resultsFleachenDaten.fleachenZuZiehen = fleachenZuZiehen[0]
...
async function getInformationsForGenerateKmlFile(productIDs,nurEinKML) {
  ...
  try {
    if (Array.isArray(productIDs) && !  nurEinKML) {
      ...
      const [infoProdukt] = await connection.execute('SELECT PRODUKT.ARTIKELNR, PRODUKT.KUNDENNUMMER, PRODUKT.STARTDATUM, PRODUKT.FLEACHENART, PRODUKT.FLAECHENNAME, TIEFE.TIEFE, K.VORNAME, K.NACHNAME, B.ABDATUM FROM PRODUKT JOIN PRODUKT_ENTHAELT_TIEFE ON PRODUKT.ARTIKELNR = PRODUKT_ENTHAELT_TIEFE.ARTIKELNR AND PRODUKT.KUNDENNUMMER = PRODUKT_ENTHAELT_TIEFE.KUNDENNUMMER JOIN TIEFE ON PRODUKT_ENTHAELT_TIEFE.TIEFENID = TIEFE.TIEFENID JOIN BEARBEITUNGSART B ON B.BEARBEITUNGSARTID = PRODUKT.BEARBEITUNGSARTID JOIN KUNDE K ON PRODUKT.KUNDENNUMMER = K.KUNDENNUMMER WHERE PRODUKT.ARTIKELNR = ? AND PRODUKT.KUNDENNUMMER = ? ', [productIDs[0], productIDs[1]]);
...
async function getBestllungenFromUser(userID) {
  ...
  try {
    ...
    LEFT JOIN
    PRODUKT_ENTHAELT_PROBE PS ON P.ARTIKELNR = PS.ARTIKELNR AND P.KUNDENNUMMER = PP.KUNDENNUMMER AND PS.PROBENARTID = 3
    WHERE O.Kundennummer = ? and (PP.PROBENSTATUS = 1 or PE.PROBENSTATUS = 1 or PS.PROBENSTATUS = 1)
    ORDER BY P.ARTIKELNR
    `, [userID]);
    cart.productsAlle = result3;
...
async function getFlächenNachBearbeitungsID(BEARBEITUNGSARTID) {
  ...
  try {
    ...
    SELECT DISTINCT fc.ARTIKELNR, fc.KUNDENNUMMER, k.VORNAME, k.NACHNAME ,k.FLAECHENFARBE,fc.FKOORDINATENIDLAT, fc.FKOORDINATENIDLNG, fc.POSITIONSPUNKT, b.ABDATUM,
    ...
    P.FLAECHENNAME, P.STARTDATUM,  P.FLEACHENART,  P.KUNDENNUMMER,  P.BEARBEITUNGSARTID, PEP.PROBENSTATUS
    FROM FLEACHENKOORDINATE fc
    JOIN  PRODUKT P ON P.ARTIKELNR = fc.ARTIKELNR AND P.KUNDENNUMMER = fc.KUNDENNUMMER
    ...
    LEFT JOIN  PRODUKT_ENTHAELT_PROBE PSmin ON P.ARTIKELNR = PSmin.ARTIKELNR AND P.KUNDENNUMMER = PSmin.KUNDENNUMMER AND PSmin.PROBENARTID = 2
    LEFT JOIN  PRODUKT_ENTHAELT_PROBE PHumus ON P.ARTIKELNR = PHumus.ARTIKELNR AND P.KUNDENNUMMER = PHumus.KUNDENNUMMER AND PHumus.PROBENARTID = 3
    LEFT JOIN  PRODUKT_ENTHAELT_PROBE PCN ON P.ARTIKELNR = PCN.ARTIKELNR AND P.KUNDENNUMMER = PCN.KUNDENNUMMER AND PCN.PROBENARTID = 4
...
async function registerUserWithFleachen(kundennummer, email, telefonnummer, hashedPassword, password, vorname, nachname, geburtsdatum, ort, plz, strasse, hausnummer, selectedOption) {
  ...
  try {
    ...
    const result = await connection.execute(
      "INSERT INTO KUNDE (Kundennummer, E_MAIL, Telefonnummer, PASSWORDUNHASHED, PASSWORD, VORNAME, NACHNAME, KundeSeit, GEODATENGEBERID, FLAECHENFARBE) VALUES (?, ?, ?, ?, ?, ?, ?, NOW(), ?,?)",
      [kundennummer, email, telefonnummer, password, hashedPassword, vorname, nachname, selectedOption,generateRandomColor()]
    );
...
function generateRandomColor() {
  ...
  do {
      ...
      console.log(hexCode)
  } while (hexCode === '#58634C'); // Wiederholen, wenn die Farbe dunkelgrün ist
  return hexCode;
}
...
async function getChangeColor(userID) {
  let connection;
  try {

    console.log(userID)
    connection = await mysql.createConnection(config);
    await connection.execute("UPDATE KUNDE SET FLAECHENFARBE = ? WHERE KUNDENNUMMER = ?",[generateRandomColor(),userID]);
